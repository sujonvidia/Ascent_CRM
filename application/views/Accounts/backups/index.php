									...
									<table class="table table-striped table-bordered table-hover">
										...
										<tbody>
											<?php foreach($allAccounts as $value){ 
												if($value->favorite == "YES" AND ($value->createdby == $id OR $value->assign_to == $id)){ ?>
								...
								<div class="box-body border-radius-none" id="myAccListView" style="display: block;">
									<div class="table-responsive" style="background: #FFF; color:#000;">
										<table class="table table-striped table-bordered table-hover">
											...
											<tbody>
												<?php foreach($allAccounts as $value){ 
													if($value->favorite == "NO" AND $value->createdby == $id AND $value->assign_to == ""){ ?>
													<tr style="text-align:left;">
														<td><a href="<?php echo site_url()."yzy-accounts/index/account_details/".$value->accountid; ?>"><?php echo $value->accountname;?></a></td>
														<!-- <td><a href="#"><?php echo $value->accountname;?></a></td> -->
														<td><?php echo $value->account_no;?></td>
        ...
        // $(".mousechange").click(function(e){  // passing down the event 
        	...
        	$.ajax({
        		...
        		success: function (data, textStatus) {
        			...
        			'<p class="r-s"><span style="font-size:10px;">Account:</span><br> ' + data.dataList[0].projectname + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Task Code:</span><br> ' + data.dataList[0].projecttaskcode + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Task Type:</span><br> ' + data.dataList[0].projecttasktype + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Task Priority:</span><br> ' + data.dataList[0].projecttaskpriority + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Task Progress:</span><br> ' + data.dataList[0].projecttaskprogress + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Task Hours:</span><br> ' + data.dataList[0].projecttaskhours + '</p>' +
        			'<p class="r-s"><span style="font-size:10px;">Account Status:</span><br> INCOMPLETE</p>' +
...
<script type = "text/javascript" >
    ...
    function searchBox(v){
    	...
    	for(var i=0;i<jqAccInfo.length;i++){
    		...
    		if(str.indexOf(key) > -1){
				...
				if(jqAccInfo[i].favorite == "YES" && (jqAccInfo[i].createdby == uid || jqAccInfo[i].assign_to == uid)){
					star_design += drawTableDesign(jqAccInfo[i].accountname, jqAccInfo[i].account_no, jqAccInfo[i].phone, jqAccInfo[i].accountid);
				}
				else if(jqAccInfo[i].favorite == "NO" && jqAccInfo[i].createdby == uid && jqAccInfo[i].assign_to == ""){
	...
	function drawDiv(datadiv){
		...
		var str = 	'<div class="col-lg-3 likeIT" id="" style="height:200px;border-radius: 8px;background-color: #FFFFFF; margin: 0px 20px 20px 0;cursor: pointer;">'+
		'<div class="row" style="padding: 13px;">'+
		'<div class="col-lg-10" >'+
		'<a href="'+acc_url+'">'+
		'<p style="color:#6E777B;padding: 10px 0px 0px 0px;font-size:18px;font-weight: bold">'+
		datadiv.accountname+'<br>'+datadiv.account_no+
		'</p>'+
		'</a>'+
		'</div>';
		str +='<div class="col-lg-1" style="padding: 8px;">';
		str +='<button  data-type="'+datadiv.star_type+'" data-id="'+datadiv.accountid+'" onclick="'+star_fun+'('+datadiv.accountid+ ')" type="button" class="btn btn-default btn-sm"><i class="fa fa-star" style="color: #'+star_color+';"></i></button>';
		str +='<button type="button"  class="btn btn-default btn-sm" onclick="oDiv(\''+datadiv.accountid+'\')"><i class="fa fa-gear"></i></button>';
		str +='<button type="button" data-archive="'+datadiv.archivestatus+'"  class="btn btn-default btn-sm" onclick="oArc('+datadiv.accountid+','+datadiv.archivestatus+','+datadiv.favorite+')"><i class="fa fa-archive"></i></button>';
		str +='</div>';
		
		str+='<div class="col-lg-12" style="color: #000000;">'+
		datadiv.phone+'<br />'+datadiv.website+
		'</div>'+
		'</div>'+
		'</div>';
		return str;
		...
		if(type == "grd"){
			...
			for(var i=0;i<jqAccInfo.length;i++){
				if(jqAccInfo[i].favorite == "YES" && (jqAccInfo[i].createdby == uid || jqAccInfo[i].assign_to == uid)){
					...
					other_design += drawDiv(jqAccInfo[i]);
				}
			}
