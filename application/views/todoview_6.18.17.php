			...
			<div id="content">
				...
				<section id="widget-grid" class="">
					<div class="row">
						...
						<div class="col-lg-9 propDiv" style="display: none">
	...
	<script>
					...
					$('.dt-assignto').each(function(i,ass_usr){
						...
						if($(ass_usr).attr('data-memid')==""){
							...
							$.each(arrAss, function(k,v){
								...
								$('.userwise-row[data-id="'+v+'"]').show('slow');
								var nowid=$.now();
	 ...
	 //    	var request = $.ajax({
		...
		function insertNewTodo(data,status){
			...
			taskDetailStr += '                              </td>';
			taskDetailStr += '                              <td class="clipTD" style="width: 33%;"><input type="text" style="color:'+HourColor+';width:100%;" class="duarationClass duSInput hrCogBtn'+item.Id+'" onchange="getDurationhr($(this).data(\'id\'),' + item.Id + ',\'task\',this)" data-id="hrduration' + item.Id + '" id="hrduration' + item.Id + '" value="'+endcColorS+'">';
			taskDetailStr += '                              </td>';
			taskDetailStr += '                          </tr>';
			taskDetailStr += '                      </table>';
			taskDetailStr += '                  </div>';
			...
			// taskDetailStr += '          <div id="subTaskcountbtnValue' + item.Id + '" style="float:left;width: 49%;margin-top: 15%;" ></div>';
			 ...
			 taskDetailStr += '<i class="fa fa-share-alt hvr-glow clasI open_newpro1 cHover" aria-hidden="true" id="" title="Share This Todo" style="position: relative;right: 70px;border-color: #ffffff;color: #ffffff;float:right;margin-top: 1%;    right: 50px;" onclick="SendInvite(\'Todo\','+item.Id+',\'tasktext\');"></i>'
		...
		var asd = 1;
        function getDurationhr(id,value,type,element){
           // console.log('getDurationhrid',$(element).val());
            var valu = parseInt( $(element).val());
            var date = $("#"+type+value).val();
            
           // console.log('getDurationhr',value,valu,type);
            if(valu != NaN){
	...
	function qtipCustomStatus(element,todo_serial,status){
        ...
        function qtipPropertiesProject(element,maindata,viewtype){
        	...
        	requestass.done(function(data){
        		...
        		floatingDiv += '                    <span id="todo_name_text'+projectID+'" class="task-properties" style="width:75% !important;margin-left: 35px;white-space: nowrap;text-overflow: ellipsis;overflow:hidden">'+taskdata.Title+'</span>';
