		...
		<script type="text/javascript">
				...
				if(pName == ""){
					...
					$.ajax({
						...
						success: function (data, textStatus) {
							...
							// design += '					</div>';
							// design +=								'<div class="gpiechart"  style="width: 70px; height: 70px;"></div>';
							design +='							<canvas id="piechart_'+data.prioTask+'" width="70" height="70"></canvas>';
					...
					success: function (data, textStatus) {
						...
						if(projectArray.length > 4){
							while(num<4){
								...
								//design +='							<canvas id="jpiechart_'+p_id+'" width="70" height="70"></canvas>';
			...
			function viewMoreProject(){
				...
				while(start<totalCount){
					...
					// design += '					</div>';
					design +=								'<div class="gpiechart" id="piechart_'+projectArray[start].Id+'" style="width: 70px; height: 70px;"></div>';
					...
					console.log('draw before:',projectArray[start].Id);
    ...
    function fun_genarate_source(source) {
		...
		// alert(srcOld);
			...
			var calendarmini=$('#calendar').fullCalendar({
					...
					// flat_startdate.setDate(moment(start).format("YYYY-MM-DD HH:mm:ss"));
					// flat_enddate.setDate(moment(end).subtract('seconds', 1).format("YYYY-MM-DD HH:mm:ss"));
