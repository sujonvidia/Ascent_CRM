	...
	<script>
	 ...
	 //    	var request = $.ajax({
		...
		function insertNewTodo(data,status){
            ...
            taskDetailStr += ' <div ondrop="drop(event)" ondragover="allowDrop(event)"  data-serial="'+item.Id+'" class="row bottom-border todoDetailDive taskserial'+item.Id+' todoRow'+todo_serial+' dd-parent" dt-sta="'+item.Status+'" style="border-left: 5px solid '+blcol+';">';
                            taskDetailStr += '                  <div class="col-lg-11 col-sm-11 col-md-11 taskRow taskRowCus" style=""> ';                  
                            taskDetailStr += '                      <div class="col-lg-1 hover" style="margin-right: -2%;">';
                            taskDetailStr += '                          <div class="row text-center">';
                            taskDetailStr += ' <div class="icon-check iconGray garay" data-status="'+item.Status+'" data-serial="'+item.Id+'" onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'To-Do\',this);" id="iconGray' + item.Id + '" style="display:'+sty2+';" ></div>';
                            taskDetailStr += ' <div data-status="'+item.Status+'" data-serial="'+item.Id+'" class="icon-check iconGreen blue" onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'To-Do\',this);" id="iconGreen' + item.Id + '" style="display:'+sty1+';" ></div>';   
                            //taskDetailStr +=	'<input class="chk-complete" type="checkbox">'                   
                            taskDetailStr += '                          </div>';
                            taskDetailStr += '                          <div class="row" id="subTaskcountbtnValue' + item.Id + '">';
                            taskDetailStr += '                          </div>';
                            taskDetailStr += '                      </div>';
                            taskDetailStr += '                      <div class="col-lg-11 hover" style="width: 93.66%;">';
                            taskDetailStr += '                          <span data-serial="' + item.Id + '" id="tasktext' + item.Id + '" class="from proName clickontitle todo-text">' + item.Title + '</span>'; 

                            taskDetailStr += '                          <p class="name check"  style="margin:0px">';  

                            taskDetailStr += ' <span class="span1">Start Date: <input style="width: 15%;" data-serial="'+item.Id+nowid+'" onclick="togglecalendar_startPro(this,$(this).data(\'c\'))" data-c = "1" type="text" id="startDatein' + item.Id +nowid+ '" name="mydate" class="datepicker customdate cls-startdate" data-dateformat="M d yy"  value="'+moment(item.Startdate).format('MMM DD YYYY')+'"></span><span class="span-divider">|</span>';

                            taskDetailStr += ' <span class="span2">Duration: <input type="text" class="duarationClass" data-nowid="'+nowid+'" onchange="getDuration($(this).data(\'id\'),' + item.Id + ',\'startDatein\',this)" data-id="duration' + item.Id +nowid+ '" id="duration' + item.Id+nowid + '" value="'+days+'"> days</span><span class="span-divider">|</span>';                            
                            taskDetailStr += ' <span class="span3" style="margin-left: 0%;">Due Date: <input data-serial="'+item.Id+nowid+'" onclick="togglecalendar_endPro(this)" type="text" id="endDatein' + item.Id+nowid + '" style="color:'+vColor+';width: 15%;" name="mydate" class="datepicker customdate cls-enddate" data-dateformat="M d yy" value="'+EnddateC+'"></span><span class="span-divider">|</span> <span style="z-index: 100;position: relative;width: 160px;display: inline-block;">Status: <span class="link_status_text'+item.Id+'  dt-todostatus" id="openstatus'+item.Id+'" data-type="'+item.Type+'" data-serial="'+item.Id+'" data-status="'+item.Status+'" >'+item.Status+'</span> </span><span class="span-divider">|</span>';  
                            taskDetailStr +=' <span onClick="userListTask(this,' + item.Id + ')">Assign To:</span> <span data-serial="'+todo_serial+'" data-memid="'+mem_ids+'" data-assid="'+tag_ids+'" style="margin-top: -5px;" class="taskTagBtnDiv dt-assignto" id="tagBtnDiv'+item.Id+'"></span>';
                            taskDetailStr += '  </p>';
                            taskDetailStr += ' </div>';    
                            taskDetailStr += '</div>';
                            taskDetailStr += '<div class="col-lg-1 col-sm-1 col-md-1 imgRow hover imgrowTask"  style="    margin-left: -2%;">';
                            ...
                            taskDetailStr += '                          <img id="openQtipProperty' + item.Id + '" title="Task Properties" class="icon-todo-menu" src="<?php echo base_url(); ?>icons/Details_Properties.png">';
                            taskDetailStr += '                      </div>';
                            taskDetailStr += '                  </div>';
			...
			// });
			 ...
			 if(item.Status == 'initiated'){
			     $("#openstatus"+item.Id).css('color','GRAY');
			 }
			 ...
			 $('#openQtipProperty'+item.Id).click(function(){
			      qtipPropertiesProject(this,item,'Todo');
			 });
			 ...
			 if(item.stickynote){                  
			     ...
			     $("#stickynote"+item.Id).attr('src','<?php echo base_url(); ?>icons/Notes_active.png');
			     if(item.notepopup==1)                     
			         $("#stickynote"+item.Id).trigger('click');
			 
			 }else{
			     $("#stickynote"+item.Id).attr('src','<?php echo base_url(); ?>icons/Notes.png');
			 }
			 
			 

			var fpstart= flatpickr("#startDatein"+item.Id+nowid, {
			     enableTime: false,
			     dateFormat: 'M-d-Y',
			     clickOpens:false,
			     ...
			     onChange: function(selectedDates, dateStr, instance) {
			         ...
			         thisValue(selectedDates[0],item.Id,'startDatein','duration','todo',nowid);

			     }
			 });

			 var fpend= flatpickr("#endDatein"+item.Id+nowid, {
			     enableTime: false,
			     dateFormat: 'M-d-Y',
			     clickOpens:false,
			     minDate:moment(item.Startdate).format('MMM-DD-YYYY'),
			     onChange: function(selectedDates, dateStr, instance) {
			         thisValue(selectedDates[0],item.Id,'endDatein','duration','todo',nowid);
			 ...
			 arr_fpstart.push({"Id":item.Id+nowid,"date":fpstart});
			 arr_fpend.push({"Id":item.Id+nowid,"date":fpend});
...
//			qtipAssignHover(newtodo.find('.ddm-assignto'),data.Id);
		...
		{ 
			...
			if($(this).text() == 'Completed'){
				...
				$('.dt-assignto').each(function(i,ass_usr){
					...
					if($(ass_usr).attr('data-assid')==""){
						...
						$.each(arrAss, function(k,v){
							...
							var fpstart= flatpickr("#startDatein"+serial+nowid, {
			     enableTime: false,
			     dateFormat: 'M-d-Y',
			     clickOpens:false,
			     ...
			     onChange: function(selectedDates, dateStr, instance) {
			         ...
			         thisValue(selectedDates[0],serial,'startDatein','duration','todo',nowid);

			     }
			 });

			 var fpend= flatpickr("#endDatein"+serial+nowid, {
			     enableTime: false,
			     dateFormat: 'M-d-Y',
			     clickOpens:false,
			     //minDate:moment(item.Startdate).format('MMM-DD-YYYY'),
			     onChange: function(selectedDates, dateStr, instance) {
			         thisValue(selectedDates[0],serial,'endDatein','duration','todo',nowid);
			 ...
			 arr_fpstart.push({"Id":serial+nowid,"date":fpstart});
			 arr_fpend.push({"Id":serial+nowid,"date":fpend});
 ...
 //    });
	...
	function qtipCustomStatus(element,todo_serial,status){
		...
		function qtipPropertiesProject(element,maindata,viewtype){
            ...
            if($(element).qtip('api') == undefined){
                ...
                var attr='properties';
                if(viewtype == 'Task' || viewtype == 'Todo'){
                    posmy= 'right center';
                    posat= 'left center';
                    ...
                    events: {
                        ...
                        show: function(event, api) {
                            ...
                            requestass.done(function(data){
                                ...
                                floatingDiv += '        <div class="panel-heading customSelect" style="height:113px;">';
                                floatingDiv += ' <div data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" class="icon-check iconGray garay iconGrayWS'+taskdata.Id+'"  onClick="makeCompleteWS(' + taskdata.Id + ',\'initiated\',\'To-Do\',this);" id="iconGray' + taskdata.Id + '" style="display:'+sty2+';position: absolute;" ></div>';
                                floatingDiv += ' <div data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" class="icon-check iconGreen blue iconGreenWS'+taskdata.Id+'" onClick="makeCompleteWS(' + taskdata.Id + ',\''+taskdata.Status+'\',\'To-Do\',this);" id="iconGreen' + taskdata.Id + '" style="display:'+sty1+';position: absolute;" ></div>';
		...
		function getDuration(id,value,type,element){
            ...
            if(valu != NaN){
                ...
                thisValue(date,value,'startDatein','duration','todo',nowid);
        ...
        function thisValue(thisDate,taskId,targetID,durationID,type,nowid){
			
			
			if(targetID == 'startDatein'){
				
				var startdate =  moment($("#startDatein"+taskId+nowid).val()).format("YYYY-MM-DD HH:mm:ss");
				var enddate = moment($("#endDatein"+taskId+nowid).val()).format("YYYY-MM-DD HH:mm:ss");
            ...
            if(days >= 0) {
             ...
             $("#"+durationID+taskId+nowid).val(days);
    ...
    function gotIt(projectID,result){
    	$.ajax({
    		...
    		dataType: "json",
    		success: function (res) {
    			$('#iconGray'+projectID).css('display','block');
    			$('#iconGreen'+projectID).css('display','none');
    			$('.icon-check[data-serial="'+projectID+'"]').attr('data-status',result);
    			$('#openstatus'+projectID).text(result);
    			$('#taskStatusLi'+projectID).text(result);
    ...
    function gotItWS(projectID,result){
        ...
        function completePro(projectID){
            $.ajax({
                ...
                success: function (res) {
                    $('#iconGray'+projectID).css('display','none');
                    $('#iconGreen'+projectID).css('display','block');
                    $('.icon-check[data-serial="'+projectID+'"]').attr('data-status','completed');
                    $('#openstatus'+projectID).text('completed');
                    $('#taskStatusLi'+projectID).text('completed');
         ...
         function makeComplete(taskID,status,type,element){
            ...
            if(status == 'completed'){
                ...
                swal({
                  ...
                  gotIt(taskID,result);
                    ...
                    swal({
                      title: "Are you sure?",
                      text: "Would you complete this to-do?",
                      type: "warning",
                      showCancelButton: true,
        ...
        function makeCompleteWS(taskID,status,type,element){
        	var status= $(element).attr('data-status');
        	if(status == 'completed'){
        		//create clone from existing element
        ...
        function setProjecttag(data,tagDivID){
            console.log('setProjecttag');
            console.log(data.tag);
            console.log(tagDivID);
            $("#tagBtnDiv"+tagDivID).empty();
            ...
            if(totalMember > 2){
                ...
                for(var i = 0;i < 2;i++){
                    ...
                    $("#tagBtnDiv"+tagDivID).append('<a id="tagLiNum'+data.tag[i].ID+tagDivID+'" title="'+data.tag[i].display_name+'" style="margin-right: 2px;margin-bottom: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">'+acronym+'</a>');
                }

                // $("#tagBtnDiv").append('<a style="margin-right: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">+'+remainingNum+'</a>');
                $("#tagBtnDiv"+tagDivID).append('<a style="margin-right: 2px;margin-bottom: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">+'+remainingNum+'</a>');
            }else{
