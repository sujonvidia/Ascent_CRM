...
<html lang="en" class="<?php echo ($page_style_result == 0)?"hidden-menu-mobile-lock":""; ?>">
	<head>
		...
		<style type="text/css">
			...
			.TtaskRow:hover, .TtaskRow.active{
				background-color: #cccccc !important;
			}
			.TtaskRow:hover .tnstitle{
				border: none;
			}
			.TtaskRow.active .tnstitle{
				border: none;
			}
			.todopropBody,.todoshareBody {
				border-radius: 8px;
				border: 1px solid #ffffff;
			}
			
			.todopropBody:before, .todoshareBody:before {
				right: 99.9%;
				top: 100px;
    	...
    	margin-top: 0px !important;
			...
			.proDiv .panel .note-toolbar .btn{
			color: #333 !important;
    		background-color: #fff !important;
    		border-color: #ccc !important;
			
			}
			.proDiv .panel .note-toolbar .btn:hover{
			color: #333 !important;
			background-color: #7d7a7a !important;
			border-color: #adadad !important; 
			}
			
			.note-toolbar .btn i,.note-toolbar .btn span{
			margin-left: -5px;
			}
			.note-toolbar .btn .note-icon-caret{
			margin-left: 0px !important;
			}
		</style>  
		<style type="text/css">
		...
		<div id="main" role="main">
			...
			<div id="content">
				...
				<section id="widget-grid" class="">
					<div class="row" style="background: rgb(230, 230, 230);">
						<div class="col-lg-12 todoDiv" style="width: 50%">
							<div class="panel panel-default proDiv">
								...
								<div class="panel-body font-color todo-font-size">
									...
									<div class="row margin-topdown">
										...
										<div id="todoViewDiv" class="col-lg-4 col-sm-4 col-md-4">
											<li  class="ddm-todoview" style="display:inline">
												<a class="dropdown-toggle dd-tog-view" data-toggle="dropdown">
													
													<span id="viewby_text"> </span>
													<i class="fa fa-caret-down"></i>
												</a>
												<ul class="dropdown-menu dropdown-change-view">
													<div class="arrow-top-right arrow-position-view"></div>
													
													<li class="active" onclick="load_todos(false,'DESC','CreatedDate',this)"><a>Create Date (DESC)</a></li>
													<li onclick="load_todos(false,'ASC','CreatedDate',this)"><a>Create Date (ASC)</a></li>
													...
													<li onclick="load_todos(false,'DESC','Assignee',this)"><a>Assignee (DESC)</a></li>
													<li onclick="load_todos(false,'ASC','Assignee',this)"><a>Assignee (ASC)</a></li>
												</ul>
											</li>
						...
						<div class="col-lg-9 propDiv" style="width: 50%;padding-left: 0px">
							
							<div class="panel panel-default proDiv" style="overflow: hidden;">
								
								<div class="panel-body font-color todo-font-size todopropBody">
									<div id="propdyn">
	...
	<i style="top: 180px;    right: 6px;" class="fa fa-close exportProject inviteClose clasI" aria-hidden="true" onClick="closePropDiv()"></i>
    <img style="top: 180px;right: 42px;" src="<?php echo base_url(); ?>/asset/img/icons/60525.png" class="exportProject inviteClose clasI sendShareTaskEmailBtn" aria-hidden="true" onClick="shareWithOther()">
	<!-- PAGE FOOTER -->
	...
	<script>
		...
		function load_todos(autostatus,order,sortname,element=false){
			...
			request.fail(function(r1, r2, r3){
				console.log(r1);
				console.log(r2);
				console.log(r3);
			});
			...
			request.done(function(response){
				...
				if(element==false){
					$('#viewby_text').text('View : Create Date (DESC) ');
					}else{
					...
					$('.dt-assignto').each(function(i,ass_usr){
						...
						if($(ass_usr).attr('data-memid')==""){
							...
							$.each(arrAss, function(k,v){
								...
								newtodo.find('[id^="startDatein"]').attr('id','startDateinAsg'+serial+nowid).attr('data-serial',serial+nowid);
								
								newtodo.find('[id^="startDateinAsg"]').click(function(){
									togglecalendar_startPro(serial+nowid);
								});
								
								newtodo.find('[id^="endDateinNew"]').attr('id','endDateinAsg'+serial+nowid).attr('data-serial',serial+nowid);
								
								newtodo.find('[id^="endDateinAsg"]').click(function(){
									togglecalendar_endPro(serial+nowid);
								});
								
								newtodo.find('.taskTagBtnDiv').attr('id','tagBtnDiv'+serial+nowid);
								
								getTagAjaxPro(serial,'Asg',nowid);
								
								//console.log('duarationClass',newtodo.find('.duarationClass'));
								...
								var fpstartasg= flatpickr("#startDateinAsg"+serial+nowid, {
									enableTime: false,
									dateFormat: 'M-d-Y',
									clickOpens:false,
									minDate:moment($("#startDateinAsg"+serial+nowid).val()).format('MMM-DD-YYYY'),
									maxDate:moment($("#endDateinAsg"+serial+nowid).val()).format('MMM-DD-YYYY'),
									
									onChange: function(selectedDates, dateStr, instance) {
										
										thisValue(selectedDates[0],serial,'startDateinAsg','duration','todo',nowid);
										
										
									}
								});
								
								var fpendasg= flatpickr("#endDateinAsg"+serial+nowid, {
									enableTime: false,
									dateFormat: 'M-d-Y',
									clickOpens:false,
				...
				if(autostatus){
					if(autoid=="new"){
						...
						$('.todoRow'+autoid).css('backgroundColor','#f1f1f1');
						$('.todoRow'+autoid+' input').css('backgroundColor','#f1f1f1');
				...
				// 	}else{
				// 	$('.todo-info-row').css({"marginLeft": "1%", "width": "100%"});
				// 	$('.todo-info-row *').removeClass('small-font-size');
				// }
				
				if(getCookie('quicklist') != ""){
					if($('.taskserial'+getCookie('quicklist')).length>0){
						//console.log('cookie user found');
						$('.taskserial'+getCookie('quicklist'))[0].click();
					}else{
						//console.log('cookie user not found');
					$('#todoInsertDiv .dd-parent')[0].click();
				}
		...
		load_todos(false,'DESC','CreatedDate');
		...
		function getTagAjaxPro(project_ID,type = 'Task',nowid=0){
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    ...
                    if( type == 'Task' ){
                        ...
                        setProjecttag(data,project_ID,nowid);
					}
                    else{
		...
		function insertNewTodo(data,status){
			...
			//Parent Div Start
			taskDetailStr += '<div onclick="openPropDyn(event)" ondrop="drop(event)" ondragover="allowDrop(event)"  data-serial="'+item.Id+'" class="dd-parent taskRow taskRowCus taskRow0604 bottom-border taskDetailDive taskserial'+item.Id+'  dt-todostatus" dt-sta="'+item.Status+'" style="float: left;width:100%;">';
			//icon Div Start
			
			taskDetailStr += '    <div style="width:100%;float:left;padding:5px"  class="TtaskRow" id="readOnlyID'+item.Id+'">';
			taskDetailStr += '      <div style="width:5%;float:left;">';
			taskDetailStr += '          <i class="fa fa-circle ql_bullet_'+item.Id+'" style="display:block;color: #'+inStartClr+';float: left; margin: 21%; padding: 0%; height: 15px; width: 15px;font-size: 18px;"></i>';
			taskDetailStr += '      </div>';

			//Main Div Start
			taskDetailStr += '      <div style="width:95%;float:left;position:relative">';
			//Main Div Upper Start
			taskDetailStr += '          <div class="slineSS" style="">';
			taskDetailStr += '              <span data-serial="' + item.Id + '" id="tasktext' + item.Id + '" class="form proName clickontitle tnstitle">' + item.Title + '</span>';
			taskDetailStr += '              <span class="ql_ago" id="ql_ago' + item.Id + '" class="">' + agostr + '</span>';
			
			taskDetailStr += '          </div>';
			
			taskDetailStr += '      </div>';

			// taskDetailStr += '      <div style="width:10%;float:right;">';
			// taskDetailStr += '          <i class="fa fa-check iconGray"  onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'Task\');" id="iconGray' + item.Id + '" style="display:'+sty2+';" ></i>';
			// taskDetailStr += '          <i class="fa fa-check iconGreen" onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'Task\');" id="iconGreen' + item.Id + '" style="display:'+sty1+';" ></i>'; 
			// taskDetailStr += '      </div>';

			// taskDetailStr += '      <div style="width:9%;float:left;margin-top: 1%;">';
			// taskDetailStr += '          <span class=""><i class="fa fa-check cusIconCom"  onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'Task\');" id="iconGray' + item.Id + '" style="display:'+sty2+';margin-left: -5px !important;" ></i><i class="fa fa-check cusIconInCom" onClick="makeComplete(' + item.Id + ',\''+item.Status+'\',\'Task\');" id="iconGreen' + item.Id + '" style="display:'+sty1+'; margin-left: -5px !important;" ></i></span>';
			// taskDetailStr += '      </div>';
	...
	<script type="text/javascript">
        ...
        function propertiesProjecttab(projectid,viewtype,parentID,taskdata,data){
            ...
            tabDetail +='   <div>';
            tabDetail +='       <div id="commentinputNEW" style="" contenteditable data-status="ProjectCmnt" data-action="0" class="summernote commentinputClas'+projectid+'"></div>';
		...
		function openPropDyn(event){
			console.log('openPropDyn',event.target);
			...
			if(offset != undefined){
				...
				$('head').append('<style>.todopropBody:before{top:'+taskDivy+'px !important;}</style>');
			}
			...
			$('.taskserial'+projectID).find('.TtaskRow').addClass('active');
			//getPropertyInfoDyn(projectID,0,'Todo');
			
			setCookie('quicklist',projectID,30);
			...
			requestass.done(function(data){
				...
				floatingDiv += '<div class="panel-heading customSelect" style="height:120px;">'; // panel heading

				floatingDiv += '<div class="proDivname" style="margin-top:0px;height:32px">'; // title info div

				floatingDiv += '<span id="todo_name_text'+projectID+'" class="task-properties" style="width:75% !important;white-space: nowrap;text-overflow: ellipsis;overflow:hidden">'+taskdata.Title;
				floatingDiv +='</span>';
				...
				floatingDiv +='<div>'; // div start
				floatingDiv +='<a class="dropdown-toggle procopyIMG hvr-glow clasI" data-toggle="dropdown" style="position: absolute;right: 90px;top: 40px;"><img style="width: 107% !important;margin-top: -24px;" src="'+base_url+'asset/icons/proCopy.png">'
				floatingDiv +='</a>';

				floatingDiv +='<ul class="dropdown-menu multi-level pull-right" style="padding-top:0px;position: absolute;top: 65px;">';

				floatingDiv +='<li onclick="convert2Task('+projectID+',0,\'duplicate\')"><a>Duplicate:</a></li>';

				//floatingDiv +='<div class="arrow-top-right"></div>';

				floatingDiv +='						<li class="dropdown-submenu pull-left">';
			

				floatingDiv +=                '<a>Move To : </a>';
				floatingDiv +=                '<ul class="dropdown-menu" style="width:200px">';
				
				$.each(allprojects, function (key, value) {
					floatingDiv+=' 									<li onclick="convert2Task('+projectID+','+value.Id+',\'move\')"><a href="#">'+value.Title+'</a></li>';
				});
				...
				floatingDiv +='						<li class="dropdown-submenu pull-left">';
				floatingDiv +=                '<a>Copy To : </a>';
				floatingDiv +=                '<ul class="dropdown-menu" style="width:200px">';
				
				$.each(allprojects, function (key, value) {
					floatingDiv+=' 									<li onclick="convert2Task('+projectID+','+value.Id+',\'copy\')"><a>'+value.Title+'</a></li>';
				});
				...
				floatingDiv +='</div>'; // div end

				floatingDiv +='<i class="fa fa-trash hvr-glow clasI" aria-hidden="true" onclick="fun_delTodo('+projectID+',\''+viewtype+'\')" style="margin-top: -6px;font-size: 14px;right: 55px;top: 40px;position: absolute;"></i>';
				...
				if(item.isShared == '1'){
					floatingDiv += '<span style="margin-top: -6px;font-size: 14px;right: 12px;top: 32px;position: absolute;" class="" data-toggle="tooltip" onclick="qtipSharedOnclick(\'Todo\','+item.Id+',\'tasktext\')" onmouseenter="qtipSharedBox(this,'+item.Id+')" data-placement="top"><i class="fa fa-share-alt hvr-glow clasI" aria-hidden="true" style="margin: 8px;"></i></span>';
				}else{
					floatingDiv += '<span style="margin-top: -6px;font-size: 14px;right: 12px;top: 32px;position: absolute;" class="" data-toggle="tooltip" onclick="SendInvite(\'Todo\','+item.Id+',\'tasktext\')" onmouseenter="qtipSharedBox(this,'+item.Id+')" data-placement="top"><i class="fa fa-share-alt hvr-glow clasI" aria-hidden="true" style="margin: 8px;"></i></span>';
				}
				...
				floatingDiv +='<span class="duSpan">Created by:'+taskdata.creator_name+'</span> |';
				floatingDiv += '              <span class="span3"><span class="duSpan dropdown-toggle pointer" aria-hidden="true" data-toggle="dropdown" aria-haspopup="true"><span >Start: <span><input type="text" id="startDatein' + taskdata.Id + '" name="mydate" style="color:'+SColor+';width: '+StartdateW+';" class="datepicker customdate" data-dateformat="M d yy"  value="'+StartdateC+'"></span></span></span>|';
				floatingDiv += '              <span class="span3" style="margin-left: 0%;"><span class="duSpan dropdown-toggle pointer" aria-hidden="true" data-toggle="dropdown" aria-haspopup="true"><span >Due by: <span style="color:'+vColor+';"><input  type="text" id="endDateinNew' + taskdata.Id + '" style="color:'+vColor+';width: '+EnddateW+';" name="mydate" class="datepicker taskDate customdate " data-dateformat="M d yy" value="'+EnddateC+'"></span></span></span>|';
				floatingDiv += '              <span data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-toggle" aria-hidden="true" style="margin-right: 0px;-webkit-box-shadow: none;box-shadow: none;"><span class="duSpan pointer" id="duraCogBtn'+taskdata.Id+'"> Duration: <input type="text" style="color:'+duColor+';width:'+wid+';background-color:transparent" class="duarationClass duSInput duraCogBtn'+taskdata.Id+'" onchange="getDuration($(this).data(\'id\'),' + taskdata.Id + ',\'startDatein\')" data-id="duration' + taskdata.Id + '" id="duration' + taskdata.Id + '" value="'+dueColorS+'"></span></span>|';
				floatingDiv += '              <span data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-toggle pointer" aria-hidden="true" id="hrCogBtn'+item.Id+'" style="margin-right: 0px;-webkit-box-shadow: none;box-shadow: none;"><span class="duSpan"> Hours: <input type="text" style="color:'+HourColor+';width: '+widHr+';font-size: 11px !important;background-color:transparent" class="duarationClass duSInput hrCogBtn'+taskdata.Id+'" onchange="getDurationhr($(this).data(\'id\'),' + taskdata.Id + ',\'task\')" data-id="hrduration' + taskdata.Id + '" id="hrduration' + taskdata.Id + '" value="'+endcColorS+'"></span></span>';
				// floatingDiv += '              <span style="z-index: 100;position: relative;" class="duSpan" ><span clas="link_status_text'+taskdata.Id+'  dt-todostatus" id="Levopenstatus'+taskdata.Id+'" data-type="'+taskdata.Type+'" data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" > Status:  </span> <span  class="link_status_text'+taskdata.Id+'  dt-todostatus deleteStatus" id="openstatus'+taskdata.Id+'" data-type="'+taskdata.Type+'" data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" >'+taskdata.Status+'</span> </span>';
				...
				floatingDiv +='<span id="member_addDiv" style="height: 32px;overflow:auto;padding-left: 5px !important;" class="pull-left duSpan">'; // members

				floatingDiv +='<span style="" onClick="TaskUserList(\'TaskMemArea\',this,' + taskdata.Id + ',\'2\')">Members:' 
				floatingDiv +='</span>';

				floatingDiv +='<span class="" id="TaskMemArea'+taskdata.Id+'">'
				floatingDiv +='</span>';

				floatingDiv +='<i class="fa fa-plus btn-primary btn-circle btnTag" onClick="TaskUserList(\'TaskMemArea\',this,' + taskdata.Id + ',\'2\')" id="addBtnTag'+taskdata.Id+'" style="padding-top: 1px !important;">'
				floatingDiv +='</i>';
				...
				floatingDiv += '<span style="z-index: 100;position: relative;margin: 6px;padding-top: 20px;top: 3px;right: 5px;" class="duSpan" ><span clas="link_status_text'+taskdata.Id+'  dt-todostatus" id="Levopenstatus'+taskdata.Id+'" data-type="'+taskdata.Type+'" data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" > Status:  </span> <span  class="link_status_text'+taskdata.Id+'  dt-todostatus deleteStatus" id="openstatus'+taskdata.Id+'" data-type="'+taskdata.Type+'" data-serial="'+taskdata.Id+'" data-status="'+taskdata.Status+'" >'+taskdata.Status+'</span> </span>';
				...
				floatingDiv += '<div id="dynprop_body" class="panel-body"  style="padding-top: 0px;">'+tabsDesignProject(projectID,viewtype,parentID,taskdata,data); // panel body
				...
				console.log('isfocus',$(event.target).hasClass("clickontitle"));

				$('.summernote').summernote({
					height: 80,
					//focus: !$(event.target).hasClass("clickontitle"),
					placeholder: 'Write a comment for this quick list',
					toolbar: [
						['style', ['style']],
        ...
        function thisValue(thisDate,taskId,targetID,durationID,type,nowid=0){
        	...
        	if(targetID == 'startDatein'){
        		...
        		var startdate =  moment($("#projectstartDateT"+taskId).val()).format("YYYY-MM-DD HH:mm:ss");
        		var enddate =  moment($("#projectendtDateT"+taskId).val()).format("YYYY-MM-DD HH:mm:ss");
        	}else if(targetID == 'startDateinAsg'){

        		var startdate =  moment($("#startDateinAsg"+taskId+nowid).val()).format("YYYY-MM-DD HH:mm:ss");
        		var enddate =  moment($("#endDateinAsg"+taskId+nowid).val()).format("YYYY-MM-DD HH:mm:ss");

        	}else if(targetID == 'endDateinAsg'){
            ...
            if(status == 'completed'){
                ...
                if(type == 'Task'){
                    ...
                    if(check_no > 0){
						...
						swal({
							title: "Are you sure?",
							text: "Would you complete this quick list?",
							type: "warning",
							showCancelButton: true,
							confirmButtonClass: "btn-danger",
							confirmButtonText: "Yes, Complete it!"
							...
							success: function (data, textStatus) {
								...
								if(data.tag.length>0){
									...
									$.each(data.tag, function (key, value) {
										...
										if(SC < 2 && SC > 0 ){
											//taskDetailStr += '<li style="background-color: #333;font-size: 12px;" class="dropdown-menu-header">Assign To</li>';
											taskDetailStr += '  <div class="clipHead cusClip">SET ASSIGNEE: <i class="fa fa-share-alt hvr-glow clasI open_newpro1 cHover" aria-hidden="true" id="" title="Assign this task to Guest(s)" style="position: absolute; padding-top: 5px; right: 25px; top: 9px; border-color: #ffffff; color: #000000;" onclick="'+shareBtnIcon+'(\'Todo\','+projectID+',\'tasktext\');"></i><i class="fa fa-times-circle cCcloeSS qtipCloseDes"></i></div>';
										}
		...
		// set tag
        function setProjecttag(data,tagDivID,nowid=""){
            ...
            if(totalMember < 1){
                $("#tagBtnDiv"+tagDivID+nowid).append('<a style="color:RED;font-size: 12px;" id="nonbtn'+tagDivID+'">[None]</a>');
                ...
                for(var i = 0;i < 2;i++){
                    ...
                    $("#tagBtnDiv"+tagDivID+nowid).append('<a  onmouseenter="qtipTaskByUser(this,'+tagDivID+','+data.tag[i].ID+','+tagDivID+')"  id="tagLiNum'+data.tag[i].ID+tagDivID+'" title="'+data.tag[i].display_name+'" style="margin-right: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">'+splitAcc+'</a>');
				}
                ...
                $("#tagBtnDiv"+tagDivID+nowid).append('<a style="margin-right: 2px;" href="javascript:void(0);" onmouseenter="qtipTaskByUser(this,'+tagDivID+',\''+remainingUserArray+'\',3)" class="btn btn-primary btn-circle">+'+remainingNum+'</a>');
				}else{
							...
							requestass.done(function(response){
								...
								if(type_id == 3){
									...
									$.each(response.task_detail, function(k,v){
										if(response.user_detail[k][0] !=undefined){
