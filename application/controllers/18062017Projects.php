...
class Projects extends CI_Controller {
    ...
    public function updateProjectDate() {
        ...
        $data['vlu'] = $vlu;
        $row = $this->db->query("SELECT MAX(id) AS `maxid` FROM `crm_modcomments` WHERE `typeID` = '" . $this->input->post("TypeID") . "'")->row();
        if ($row) {
            $maxid = $row->maxid;
        }
    ...
    public function TaskMakeCompleteWS() {
        ...
        $maxid = 0;
        $row = $this->db->query("SELECT MAX(id) AS `maxid` FROM `crm_modcomments` WHERE `typeID` = '" . $taskID . "'")->row();
        if ($row) {
            $maxid = $row->maxid;
        }
    ...
    public function tagUserWithStory() {
        ...
        $maxid = 0;
        $row = $this->db->query("SELECT MAX(id) AS `maxid` FROM `crm_modcomments` WHERE `typeID` = '" . $projectID . "'")->row();
        if ($row) {
            $maxid = $row->maxid;
        }
    ...
    public function updateDueDate() {
        if ($this->session->userdata('yeezyCRM')) {
            ...
            $row_parent = $this->db->get_where("crm_activity", array("Id" => $this->input->post('parentID')))->row();

            $row_rel = $this->db->get_where("crm_activity", array("Id" => $this->input->post('type_id')))->row();

            $relatedTo = $this->input->post('parentID');
            ...
            if ($ChangeType == "SubTask") {
                ...
                if (COUNT($array['taskSubtask']) > 0) {
                    foreach ($array['taskSubtask'] as $keysub => $valsub) {
                        ...
                        if (new DateTime($valsub->Enddate) > new DateTime($_POST['DueDate'])) {
                            ...
                            if ($valsub->CreatedBy != $data['id']) {
                                ...
                                $array['upstatus_sub'][$key] = $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $_POST['DueDate']), array('Id' => $valsub->Id));
                            }
                        }
                    }
                }
            ...
            if (new DateTime($row_parent->Enddate) < new DateTime($_POST['DueDate'])) {
                ...
                if ($_POST['CreatedBy'] != $data['id']) {
                    ...
                    $arr['upstatus'] = $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $_POST['DueDate']), array('Id' => $_POST['parentID']));
                }
            }
