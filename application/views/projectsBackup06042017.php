...
<body id="projectBody" class="<?php echo ($page_style_result == 0)?"hidden-menu":""; ?>">
		...
		<div id="main" role="main">
			...
			<div id="content">
				...
				<i class="fa fa-caret-right slide-toggle" id="rarrow" style="position: absolute;left: -1px;top: 15px;display: none;cursor: pointer;font-size: 35px;color: #4d4d4d;"></i>
				<!-- widget grid -->
	...
	<script>
        ...
        $(".slide-toggle").click(function(){
            ...
            if($("#larrrow").is(":visible")){
                ...
                $(".excolCustom").css('width','2%');
                $(".excolCustom").css('margin-right','0%');

            }else if($("#rarrow").is(":visible")){
                $("#rarrow").hide();
                $(".TaskListDiv").removeAttr('style');
                $(".taskDiv").removeClass('change');
                $(".taskDetailDive ").css('padding-bottom','0%');
                $(".taskDetailDive ").css('padding-bottom','0%');
                $(".taskRow").css('margin-bottom','-1%');
                $(".excolCustom").css('width','8.33333333%');
                $(".excolCustom").css('margin-right','-2%');
            }
		...
		function setProjecttag(data,tagDivID){
            ...
            if(totalMember > 2){
                ...
                for(var i = 0;i < 2;i++){
                    ...
                    var acronym = matches.join(''); 
                    // $("#tagBtnDiv").append('<a title="'+data.tag[i].display_name+'" style="margin-right: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">'+acronym+'</a>');
                    $("#tagBtnDiv"+tagDivID).append('<a id="tagLiNum'+data.tag[i].ID+tagDivID+'" title="'+data.tag[i].display_name+'" style="margin-right: 2px;" href="javascript:void(0);" class="btn btn-primary btn-circle">'+acronym+'</a>');
                }
            ...
            var vColor = 'inherit';
                ...
                if(data.allTask.length > 0){
                    ...
                    $.each(data.allTask, function (i, item) {
                            ...
                            taskDetailStr += '                  <div class="col-lg-11 col-sm-11 col-md-11 taskRow taskRowCus" style="margin-bottom:-1%;"> ';                  
                            taskDetailStr += '                      <div class="col-lg-1 hover excolCustom" style="z-index: 100;height: 55px;background: rgb(255, 255, 255);margin-right: -2%;">';
                ...
                success: function (data_st, textStatus) {
                    ...
                    if(data_st.allSubTask.length > 0){
                        $.each(data_st.allSubTask, function (index, value) {
                            ...
                            subtaskList +='             <div class="col-lg-1" style="margin-right: -3%;">';
                            subtaskList +='                 <div class="row text-center">';
                            //subtaskList +='                       <img class="icon-check" src="icons/Checkmark.png">';    
                            ...
                            subtaskList += '                    <span style="margin-left: 0%;color: rgb(128, 128, 128);">Due Date: <input  style="width:11.5% !important;" type="text" onclick="togsubcalend(' + value.Id + ')" id="SubendDatein' + value.Id + '" style="color:'+vColor+';" name="mydate" class="datepicker customdate" data-dateformat="M d yy" value="'+EnddateC+'"> | <span style="    z-index: 100;position: relative;">Status: <span clas="link_status_text'+value.Id+'  dt-todostatus" id="openstatus'+value.Id+'" data-type="'+value.Type+'" data-serial="'+value.Id+'" data-status="'+value.Status+'" >'+value.Status+'</span></span>';                  
                            subtaskList += '                </p>';
                            subtaskList +='             </div>';    
                            subtaskList +='         </div>';                    
                            ...
                            subtaskList +='                     <img class="icon-todo-menu dropbtn" src="icons/Details_Properties.png" id="properties'+value.Id+'">';
                            ...
                            if(value.Status == 'initiated'){
                                $("#openstatus"+value.Id).css('color','GRAY');
                            }
                    ...
                    events: {
                        ...
                        show: function(event, api) {
                            ...
                            requestass.done(function(data){
                                ...
                                floatingDiv += '                              <div class="icon-check iconGray garay iconGrayWS'+taskdata.Id+'"  onClick="makeCompleteWS(' + taskdata.Id + ',\'initiated\',\'Task\');" id="iconGray' + taskdata.Id + '" style="display:'+sty2+';position: absolute;" ></div>';
