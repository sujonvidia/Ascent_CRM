...
<script type="text/javascript">
	...
	function updateAssigneeImg(_this,todo_serial){
		...
		if(newass !=null){
			...
			if(newass.length>1){
				$('.dt-assignto[data-serial="'+todo_serial+'"] img').attr('src',base_url+"asset/icons/gi32.png");

				$("#clickIconlist"+todo_serial).attr('src',base_url+"asset/icons/gi32.png");
			}
			else{
				$('.dt-assignto[data-serial="'+todo_serial+'"] img').attr('src',base_url+"asset/icons/Profile.png");

				$("#clickIconlist"+todo_serial).attr('src',base_url+"asset/icons/Profile.png");
			}
	...
	function todoAttachDesign(){
		...
		tabDetail +='			<div class="col-lg-5"><img class="" src="'+base_url+'asset/icons/attachIcon.png"> <span style="color: #c5c5c5;font-size: 15px;">Example File</span></div>';
		tabDetail +='			<div class="col-lg-3 attachMid">';
		tabDetail +='				<img class="icon-todo-menu" src="'+base_url+'asset/icons/Star.png">';
		tabDetail +='				<img class="icon-todo-menu" src="'+base_url+'asset/icons/Profile.png">';
	...
	function propertiesTodoOne(projectid,viewtype,parentID,taskdata){
		...
		tabDetail +='         <a class="dropdown-toggle" data-toggle="dropdown" style="right: 43px;position: absolute;"><img src="'+base_url+'asset/icons/proCopy.png"></a>';
		...
		tabDetail +='         <a onclick="fun_delTodo('+projectid+','+viewtype+')" style="right: 6px;position: absolute;"><img src="'+base_url+'asset/icons/proDlt.png"></a>';
		tabDetail +='     </div>';
  ...
  //       	tabDetail += '</td><tr></table><span style="display:none;" id="hourtimer'+projectid+'"></span>';
	...
	function propertiesTodoTwo(){
		...
		var tabDetail ='  <div class="row">';
		tabDetail +='    <div class="col-lg-12">';
		tabDetail +='         <a href=""><img src="'+base_url+'asset/icons/proDlt.png"></a>';
		tabDetail +='         <a href=""><img src="'+base_url+'asset/icons/proCopy.png"></a>';
		tabDetail +='     </div>';
	...
	$(document).on('click','.pointer',function(e){
		if($("#"+e.target.firstElementChild.id).val() == '[None]'){
			$("#"+e.target.firstElementChild.id).val('');
			$("#"+e.target.firstElementChild.id).focus();
			$("#"+e.target.firstElementChild.id).css('color','#000000');
		}else{
			$("."+e.target.id).focus();  
			$("."+e.target.id).select();
		}
	...
	function convert2Task(todo_id,pro_id,status){
				...
				if (data.allFiles.length > 0) {
					$.each(data.allFiles, function (key, value) {
						...
						if(data.allFiles[key].HasStar == 'YES'){
							var icon = base_url+'asset/icons/YStar.png';
							}else{
							icon = base_url+'asset/icons/Star.png';
						}
						...
						tabDetail +='           <div class="col-lg-5"><img class="" src="'+base_url+'asset/icons/attachIcon.png"> <a class="downloadHover" href="<?php echo base_url() ?>'+downloadFolder+'/'+data.parentfolder[0].name+'/'+data.allFiles[key].name+'" download><span style="font-size: 15px;" id="fileoriname007'+data.allFiles[key].id+'">'+data.allFiles[key].original_name+'</span></a></div>';
		            ...
		            tabDetail = '          <button onclick="$(\'#fileinputAttach\').trigger(\'click\');" class="css3button"> Add Files</button><img src="'+base_url+'asset/icons/nofile.png" id="nofileImg" class="nofileImg">';
		            $("#attachListDiv").append(tabDetail);
		        }
			},
			error: function (jqXHR, textStatus, errorThrown) {
	...
	function change_projectstatus(status,serial,el){
		...
		requestass.done(function(response){
			...
			if(status=='completed'){
				...
				$('.ql_bullet_'+serial).css('color','#54ce3c');

				$('.icon-check[data-serial="'+serial+'"]').attr('data-status','completed');
				$("#taskcIcon"+serial).css('color','#54ce3c');
				if(response.HasParentId > 0){
					$("#taskcIcon"+response.HasParentId).css('color','#54ce3c');
				...
				$('.ql_bullet_'+serial).css('color','#bfbfbf');

				$("#taskcIcon"+serial).css('color','#bfbfbf');
				if(response.HasParentId > 0){
					$("#taskcIcon"+response.HasParentId).css('color','#bfbfbf');
				}
			...
			if(status == 'none'){
                ...
                $("#openstatus"+serial).css('color','ORANGE').css('font-size','9px');
            }else if(status == 'canceled'){
                $("#openstatus"+serial).css('color','RED');
            }else{
