...
class Projects extends CI_Controller {
    ...
    public function getproject() {
        ...
        foreach ($array['projects'] as $key => $value) {
            ...
            $OverdueTask = $this->Modulemodel->getAllMyTaskOverdue($data['id'], $value->Id);
            $CompleteTask = $this->Modulemodel->getAllMyTaskComplete($data['id'], $value->Id);
            ...
            array_push($array['OverdueTask'], $OverdueTask);
            array_push($array['CompleteTask'], $CompleteTask);
    ...
    // dipok for top bar tag @26-7-2017
    public function getTopbartag() {
        $tagArray = array();
        $project_ID = $_POST["project_ID"];
        $tagArray['tag'] = $this->Modulemodel->gettagprojectusers($project_ID, '2');
        $tagArray['status'] = $this->Modulemodel->getProjectStatus($project_ID);
        //$tagArray['totalTask'] = $this->Modulemodel->countTask(array('projectid'=>$project_ID));
        header('Content-Type: application/json');
        echo json_encode($tagArray);
    ...
    public function updateTaskDate() {
        ...
        if (isset($_POST['taskID'])) {
            ...
            $TaskparentID = $this->db
                            ...
                            ->row()->HasParentId;
            $CreatedByUserId = $this->db
                            ->get_where('crm_activity', array('Id' => $_POST["taskID"]))
                            ->row()->CreatedBy;
            $DueDateNew = $_POST['enddate'];
            ...
            if (strtotime($parentenddate) <= strtotime($taskenddate)) {
                ...
                // exceed parent end date 
                 if (new DateTime($EnddateParent) < new DateTime($_POST["enddate"])) {
                    // 0 : needs approval, 1 : make approval
                    if ($CreatedByUserId != $data['id']) {
                        $approval=0;

                    }else{
                        $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $DueDateNew), array('Id' => $TaskparentID));
                        $approval=1;
                    }
                ...
                $inputnot = array(
                    ...
                    'status' => '0',
                    'title' => $title[0]->Title,
                    'body' => date('Y-m-d', strtotime($_POST["startdate"]))." to ".$taskenddate,
                    'replay_msg' => '',
                    'createdby' => $data['id']
                );
                ...
                $type_id = $_POST["taskID"];
                $ChangeType = $_POST['this_type'];
                ...
                //         $data['notification_for'] = $this->Modulemodel->insertData("crm_notification", $inputtoast);

                //         file_put_contents("temp/notification_for.txt", $data['notification_for']);
                //     } else {

                //         $data['upstatus'] = $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $DueDateNew), array('Id' => $relatedTo));
                //     }
                // }
    ...
    public function saveSubTaskNew() {
        if ($this->session->userdata('yeezyCRM')) {
            ...
            $this->Modulemodel->updateOneData("crm_activity", array('Status' => 'on hold'), array('Id' => $_POST["taskId"]));
    ...
    public function TaskMakeComplete() {
        ...
        $type = $this->input->post('type');
        $sessionData = $this->session->userdata('yeezyCRM');
        $activity = $this->db->select("Type,HasParentId")->get_where("crm_activity", array("Id" => $taskID))->result();
        if($activity[0]->Type == 'SubTask'){
            if($type == 'completed'){
                ...
                if ($this->Modulemodel->updateOneData("crm_activity", array('Status' => $type), array('Id' => $taskID)) === TRUE) {
                    $this->Modulemodel->updateOneData("crm_activity", array('Status' => $type), array('Id' => $activity[0]->HasParentId));
                   $data['msg'] = "Done";
                } else {
                    $data['msg'] = "Fail";
                }
    ...
    public function getNotificationStatusAll() {
        ...
        $array['changedDateListST'] = array();

        // $array['getAllToastTask'] = array();
        // $array['getAllToastSubTask'] = array();
        ...
        foreach ($array['projects'] as $key => $value) {
            ...
            $changedDateListP = $this->Modulemodel->allNotifList(1, $value->Id, 'ProjectDatechange', $data['id']);
            $commentList1 = $this->Modulemodel->allNotifList(1, $value->Id, 'ProjectCmnt', $data['id']);
            $ProjectAttachment = $this->Modulemodel->allNotifList(1, $value->Id, 'ProjectAttachment', $data['id']);
            $getAllStatusUpdate = $this->Modulemodel->allNotifList(1, $value->Id, 'ProjectCmntStatusUpdate', $data['id']);
            ...
            foreach ($array['allTask'] as $k => $v) {
                ...
                $changedDateListT = $this->Modulemodel->allNotifList(1, $v->Id, 'TaskDatechange', $data['id']);
                $TaskHourchange = $this->Modulemodel->allNotifList(1, $v->Id, 'TaskHourchange', $data['id']);
                $TaskAttachment = $this->Modulemodel->allNotifList(1, $v->Id, 'TaskAttachment', $data['id']);
                ...
                $array['allSubTask'] = $this->Modulemodel->getAllprojectSubTasks($data['org_id'], $data['id'], $v->Id, 'DESC');
                
                foreach ($array['allSubTask'] as $ky => $va) {
                    $changedDateListST = $this->Modulemodel->allNotifList(1, $va->Id, 'SubTaskDatechange', $data['id']);
                    array_push($array['changedDateListST'], $changedDateListST);
                }

            }
        }

        $qry = "select * from crm_notification where (`type` LIKE '%Toast%') and user_id=".$data['id']." and status=0";

        $array['getAllToasts']  = $this->db->query($qry)->result();
        ...
        // $array['getActivityNot'] = $this->db->select("parent,typeid")->get_where("crm_feed_temp_tbl", array("userid" => $data['id']))->result();
    ...
    public function getCommentForProjects() {
        ...
        //         $array['allComm'].push(array('msgid'=> $row->id, 'groupId'=> $fid, 'drawNew' => 'yes', 'type' => 'right', 'msg' => base64_decode($row->msg), 'time' => $row->time, 'msgtype' => $row->msg_type, 'msg_status'=> $row->status));
        //     }
        ...
        //         $array['allComm'].push(array('msgid'=> $row->id, 'groupId'=> $fid, 'drawNew' => 'yes', 'type' => 'left', 'msg' => base64_decode($row->msg), 'time' => $row->time, 'msgtype' => $row->msg_type, 'fimg'=>$finfo[0]->img, 'fname'=>$finfo[0]->display_name, 'msg_status'=> $row->status));
        //     }
        // }
        // if(filter_var($fid, FILTER_VALIDATE_EMAIL) AND $fid == $sessionData['user_email']){
        //     $this->db->where($where);
    ...
    public function deleteFileUnseen() {
        ...
        $title = $this->db->select("Status")->get_where("crm_activity", array("Id" => $this->input->post('projectID')))->result();
        $array['status'] = $title[0]->Status;
    ...
    public function delCommentPro() {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');

        if ($this->db->delete('crm_message', array('id' => $this->input->post("id")))) {
            $array = true;
        } else {
            $array = false;
        }
    ...
    public function approveToasts() {
        if ($this->session->userdata('yeezyCRM')) {
            ...
            $row_typeid = $this->db->
            get_where("crm_activity", array("Id" => $row_notid->type_id))->row();

            $row_related = $this->db->
            get_where("crm_activity", array("Id" => $row_notid->relatedTo))->row();
            
            try {

                if ($row_notid->type == 'SubTaskDatechange') {
                     $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $row_typeid->Enddate), array('Id' => $row_notid->relatedTo));
                }

                if ($row_notid->type == 'TaskDatechange') {
                     $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $row_typeid->Enddate), array('Id' => $row_notid->relatedTo));
                }
            ...
            $data['response2'] = $this->Modulemodel->updateOneData("crm_notification", array("status" => 1,"approval"=> 1), array('ID' => $tid));
    ...
    public function rejectToasts() {
        if ($this->session->userdata('yeezyCRM')) {
           ...
           if ($row_notid->type == 'SubTaskDatechange') {
                $this->Modulemodel->updateOneData("crm_activity", array('Enddate' => $row_related->Enddate), array('Id' => $row_notid->type_id));
           }
